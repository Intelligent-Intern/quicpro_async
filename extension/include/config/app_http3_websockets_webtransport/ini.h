/*
 * =========================================================================
 * FILENAME:   include/config/app_http3_websockets_webtransport/ini.h
 * PROJECT:    quicpro_async
 * AUTHOR:     Jochen Schultz <jschultz@php.net>
 *
 * WELCOME:    Any memory can be erased. A ghost can be hacked.
 *
 * PURPOSE:
 * This header file declares the internal C-API for handling the `php.ini`
 * entries of the application protocols configuration module.
 * =========================================================================
 */

#ifndef QUICPRO_CONFIG_APP_PROTOCOLS_INI_H
#define QUICPRO_CONFIG_APP_PROTOCOLS_INI_H

/**
 * @brief Registers this module's php.ini entries with the Zend Engine.
 */
void qp_config_app_http3_websockets_webtransport_ini_register(void);

/**
 * @brief Unregisters this module's php.ini entries from the Zend Engine.
 */
void qp_config_app_http3_websockets_webtransport_ini_unregister(void);

#endif /* QUICPRO_CONFIG_APP_PROTOCOLS_INI_H */
